...
unsigned int opticalDepthTexture;
std::unique_ptr<Camera> camera;
auto startTime = std::chrono::high_resolution_clock::now(), prevTime = std::chrono::high_resolution_clock::now(), lastSecondTime = std::chrono::high_resolution_clock::now();
unsigned int frameBuf, outTexture;
std::array<std::unique_ptr<Planet>, 8UL> planets;
float mtime;
...
void mainloop()
{
    ...
    mtime += dt; // mtime is sum of dt so that we can slow mtime and rewind it
    ...
    glUniform1f(glGetUniformLocation(program, "mtime"), mtime);
    ...
    glUniform1f(glGetUniformLocation(UIprogram, "mtime"), mtime);
