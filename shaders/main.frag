...
vec3 shadeSphere(vec3 rayDir, vec3 rayPos, vec3 spherePos, float radius, vec3 lightSource, vec3 clr)
{
    ...
    return sunCoronaStrength * clr;//max(dot(normalize(pos - spherePos), normalize(lightSource - pos)), 0.0) * 0.8 * clr + 0.2 * clr;
}
...
void main()
{
    ...
    totalLight += shadeSphere(rayDir, -cameraPos, spherePos, 1.0, lightSource, sunColor);
